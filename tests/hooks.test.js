...
test("pre-push hook defaults local integration tests to Codex only", () => {
  const dir = mkdtempSync(join(tmpdir(), "ask-hook-test-"));
  try {
    ...
    writeFileSync(
      ...
      [
        "#!/bin/sh",
        "printf '%s\\t%s\\t%s\\n' \"$*\" \"${ASK_BIN:-}\" \"${ASK_INTEGRATION_AGENTS:-}\" >> \"$ASK_HOOK_CAPTURE\"",
        "exit 0",
        "",
      ].join("\n"),
      { mode: 0o755 },
    ...
    assert.deepEqual(calls, [
      ["run build", "", ""],
      ["run test:integration:local", join(repoRoot, "bin", "ask.js"), "codex"],
    ]);
  } finally {
...
test("pre-push hook can opt into all local integration agents", () => {
  ...
  try {
    ...
    assert.deepEqual(calls, [
      ["run build", "", ""],
      ["run test:integration:local", join(repoRoot, "bin", "ask.js"), "all"],
    ]);
  } finally {
    rmSync(dir, { force: true, recursive: true });
  }
